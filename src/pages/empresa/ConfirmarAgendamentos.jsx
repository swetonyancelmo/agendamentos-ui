...
const STATUS_CONFIG = {
  ...
  CONFIRMED: { label: "Confirmado", style: "bg-blue-50 text-blue-700" },
  REJECTED:  { label: "Rejeitado",  style: "bg-red-50 text-red-700" },
  CANCELLED: { label: "Cancelado",  style: "bg-slate-100 text-slate-600" },
  COMPLETED: { label: "Concluído",  style: "bg-green-50 text-green-700" },
};
...
function CardAgendamento({ item, onConfirmar, onRejeitar, feedbackId }) {
  ...
  return (
    <div className={`bg-white border rounded-xl p-5 transition-all duration-300
      ${isFeedback ? "border-blue-300 ring-1 ring-blue-300" : "border-slate-200"}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-600 mt-0.5">ID: {item.id?.slice(0, 8)}…</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3 mt-4">
        <div>
          ...
          <p className="text-xs font-medium text-slate-700 mt-0.5">{formatarData(item.appointmentDate)}</p>
        </div>
        <div>
          <p className="text-xs text-slate-600">Horário</p>
          <p className="text-xs font-medium text-slate-700 mt-0.5">{item.startTime} – {item.endTime}</p>
        </div>
        <div>
          <p className="text-xs text-slate-600">Status</p>
          <p className="text-xs font-medium text-slate-700 mt-0.5">{statusInfo.label}</p>
        </div>
      </div>
      ...
      {isPending && (
        <div className="flex gap-2 mt-4 pt-4 border-t border-slate-100">
          ...
          <button
            ...
            className="text-xs font-semibold border border-slate-200 text-slate-600 hover:border-red-300 hover:text-red-600 px-4 py-2 rounded-lg transition cursor-pointer"
          >
            Rejeitar
          </button>
        </div>
    ...
    <main id="main-content" className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-4xl mx-auto px-5 pt-28 pb-16">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-8">
          ...
          {pendentes > 0 && (
            <span className="inline-flex items-center gap-1.5 bg-amber-50 text-amber-700 text-xs font-semibold px-3 py-1.5 rounded-lg border border-amber-200">
        ...
        <div className="mb-4">
          <input
            ...
            className="w-full px-4 py-2.5 border border-slate-200 rounded-lg text-sm bg-white placeholder-slate-500 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition"
          />
        </div>
